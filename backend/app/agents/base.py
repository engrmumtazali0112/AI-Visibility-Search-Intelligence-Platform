...
Supports: Anthropic Claude (primary), Gemini, OpenAI, and Mock mode.
"""
...
class BaseAgent:
    ...
    def _get_client(self):
        ...
        if api_key and api_key not in ("sk-ant-demo-key", "sk-ant-demo"):
            try:
                import anthropic
    ...
    def _call_llm(self, user_prompt: str, max_tokens: int = 2000) -> str:
        ...
        # Anthropic
        if hasattr(client, 'messages') and hasattr(client, 'messages') and not hasattr(client, 'GenerativeModel'):
            try:
                ...
                if usage is not None:
                    self.last_tokens_used = (getattr(usage, 'input_tokens', 0) or 0) + (
                ...
                raise AgentError(self.agent_name, f"Anthropic API error: {e}")
        ...
        if hasattr(client, 'GenerativeModel'):
            try:
                ...
                raise AgentError(self.agent_name, f"Gemini API error: {e}")
        ...
        if hasattr(client, 'ChatCompletion'):
            try:
                ...
                raise AgentError(self.agent_name, f"OpenAI API error: {e}")
